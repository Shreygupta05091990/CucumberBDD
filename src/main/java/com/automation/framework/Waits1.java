...
public class Waits1{
	public Logger log=Logger.getLogger(Waits1.class);
	...
	public static void setImplicitWait(int time){
		Base1.driver.manage().timeouts().implicitlyWait(time, TimeUnit.SECONDS);
	}
	
	
	public static void waitUntilElementLocated(int time,WebElement element){
		WebDriverWait wait=new WebDriverWait(Base1.driver,time);
	...
	public static synchronized Object execJavascript(String script, Object... args) {
        JavascriptExecutor scriptExe = ((JavascriptExecutor)Base1.driver);
    ...
    public static void waitUntil(BooleanSupplier condition, int seconds) {
        new WebDriverWait(Base1.driver, seconds).until((WebDriver driver) -> condition.getAsBoolean());
    }
